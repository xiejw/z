...
type Table[T any] struct {
	nodes          []Node[T] // All dancing link nodes (headers and options).
	totalNodeCount uint      // Total count of nodes (hard cap).
}
...
type Node[T any] struct {
	...
	IdD         uint        // Id of the down node.
	VerticalCol VerticalCol // Column information.
	Udp         T           // User defined data.
}
...
func New[T any] (total_header_count uint, total_option_count uint)  Table[T] {
	return nil
}
...
//	        Table {
//	            nodes: nodes,
//	            totalNodeCount: reserved_node_count,
//	        }
//	    }

	    // Link the node `Id` into table after node `end` (horizontal double link).
// 	    func linkLR[T any](nodes []Node[T], end uint, id uint) {
// 		    IdR := nodes[end].IdR;
// 		    p := &mut nodes[id];
// 		    p.IdL = end;
// 		    p.IdR = IdR;
...
//	            IdU: id,
//	            IdD: id,
//	            VerticalCol: VerticalCol { ColHeaderCol: 0 },
//	            udp: None,
//	        }
//	    }
//	}
