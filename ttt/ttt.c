...
//
// If 1, random games against a good NN player, rather than random guess.
#define PLAY_RANDOM_GAME_AGAINST_NN 0
...
forward_pass( NeuralNetwork *nn, float *inputs )
{
        ...
        for ( int i = 0; i < NN_HIDDEN_SIZE; i++ ) {
                ...
                for ( int j = 0; j < NN_INPUT_SIZE; j++ ) {
                        sum +=
                            inputs[j] * nn->weights_ih[j * NN_HIDDEN_SIZE + i];
                }
        ...
        for ( int i = 0; i < NN_OUTPUT_SIZE; i++ ) {
                ...
                for ( int j = 0; j < NN_HIDDEN_SIZE; j++ ) {
                        nn->raw_logits[i] +=
                            nn->hidden[j] *
                            nn->weights_ho[j * NN_OUTPUT_SIZE + i];
                }
        }
...
display_board( GameState *state )
{
        for ( int row = 0; row < 3; row++ ) {
                ...
                printf( "%c%c%c ", state->board[row * 3],
                        state->board[row * 3 + 1], state->board[row * 3 + 2] );
...
check_game_over( GameState *state, char *winner )
{
        ...
        // Check diagonals.
        if ( state->board[0] != '.' && state->board[0] == state->board[4] &&
             state->board[4] == state->board[8] ) {
                *winner = state->board[0];
                return 1;
        }
        if ( state->board[2] != '.' && state->board[2] == state->board[4] &&
...
get_computer_move( GameState *state, NeuralNetwork *nn, int display_probs )
{
        ...
        if ( display_probs ) {
                ...
                for ( int row = 0; row < 3; row++ ) {
                        for ( int col = 0; col < 3; col++ ) {
                                ...
                                if ( pos == highest_prob_idx ) {
                                        printf( "*" );  // Highest probability
                                                        // overall.
                                }
                                if ( pos == best_move ) {
                                        printf(
                                            "#" );  // Selected move (highest
                                                    // valid probability).
                                }
                                printf( " " );
                        }
                ...
                for ( int i = 0; i < 9; i++ ) total_prob += nn->outputs[i];
...
void
backprop( NeuralNetwork *nn, float *target_probs, float learning_rate,
...
void
learn_from_game( NeuralNetwork *nn, int *move_history, int num_moves,
                 int nn_moves_even, char winner )
{
        ...
        for ( int move_idx = 0; move_idx < num_moves; move_idx++ ) {
                ...
                /* Here we can't really implement temporal difference in the
                 * strict reinforcement learning sense, since we don't have an
                 * easy way to evaluate if the current situation is better or
                 * worse than the previous state in the game.
                 *
                 * However "time related" we do something that is very effective
                 * in this case: we scale the reward according to the move time,
                 * so that later moves are more impacted (the game is less open
                 * to different solutions as we go forward).
                 *
                 ...
                 * different values.
                 *
                 * LEARNING OPPORTUNITY: Temporal Difference in Reinforcement
                 * Learning is a very important result, that was worth the
                 * Turing Award in 2024 to Sutton and Barto. You may want to
                 * read about it. */
                ...
                for ( int i = 0; i < NN_OUTPUT_SIZE; i++ ) target_probs[i] = 0;

                /* Set the target for the chosen move based on reward: */
                if ( scaled_reward >= 0 ) {
                        /* For positive reward, set probability of the chosen
                         * move to 1, with all the rest set to 0. */
                        target_probs[move] = 1;
                } else {
...
play_game( NeuralNetwork *nn )
{
        ...
        learn_from_game( nn, move_history, num_moves, state.nn_symbol ? 1 : 0,
                         winner );
}
                  ...
                  char *nn_symbol )
{
        ...
        *nn_symbol = state.nn_symbol;
        ...
        while ( !check_game_over( &state, &winner ) ) {
                ...
                if ( state.current_p !=
                     state.nn_symbol ) {  // Random player's turn )
                        move = PLAY_RANDOM_GAME_AGAINST_NN
                                   ? get_computer_move( &state, nn, 0 )
...
train_against_random( NeuralNetwork *nn, int num_games )
{
        ...
        for ( int i = 0; i < num_games; i++ ) {
                ...
                // Accumulate statistics that are provided to the user (it's
                // fun).
