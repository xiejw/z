...
\urldef{\codeAlgoBDPE}\url{https://github.com/xiejw/z/blob/b3106f8892d9265f92817459c079e5798d0395b4/taocp/v4_7.2.2.2.algo_b_satisfiability_by_watching/cmd/test_main.cc#L8-L116}
\urldef{\codeAlgoBWatch}\url{https://github.com/xiejw/z/blob/b3106f8892d9265f92817459c079e5798d0395b4/taocp/v4_7.2.2.2.algo_b_satisfiability_by_watching/src/sat_watch.cc#L99-L236}
...
\end{verbatim}
With all the problem setup now, we could map the problem into SAT\sidenote{Full code
can be found at \codeAlgoBDPE.}:
\begin{itemize}
...
\paragraph{Satisfiability by Watching} Algorithm B\sidenote{Code can be found
at \codeAlgoBWatch.} is a particularly elegant optimization within this domain.
In a clause consisting of a disjunction of literals, only one variable needs to
be ``watched'' to monitor whether the clause is satisfied or at risk of
becoming a conflict. This approach makes backtracking highly efficient, as the
solver only needs to visit clauses containing the specific variable being
unassigned, rather than scanning all clauses. Furthermore, if a watched literal
becomes false, the solver can cheaply update the watch to another non-false
literal. By leveraging the inherent structure of Conjunctive Normal Form (CNF),
this mechanism significantly reduces the overhead of undo operations during
search.
