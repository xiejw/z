...
class ResNetModel(torch.nn.Module):
    ...
    # A residual component for ResNet
...
# TEST the jit trace with cc.
batch_size = 8
input_example = torch.rand(8, 3, 15, 15)

model = build_resnet_model()

traced_script_module = torch.jit.trace(model, input_example)
traced_script_module.save("/tmp/g5_traced_resnet_model.pt")

output = traced_script_module(torch.ones(1, 3, 15, 15))
print(output)
