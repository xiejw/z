...
static int
read_u32_be( FILE *f, uint32_t *val_out, struct err_stack *stk )
{
        unsigned char buf[4];
                                 ...
                                 struct err_stack *stk ) )[HERMES_PIXELS]
{
        ...
        if ( read_u32_be( f, &magic, stk ) || read_u32_be( f, &n, stk ) ||
             read_u32_be( f, &rows, stk ) || read_u32_be( f, &cols, stk ) ) {
                forge_err_emit(
                    stk, "load_all_images: failed reading header of '%s'",
                    path );
                goto exit;
        }
        if ( magic != 0x00000803u ) {
                forge_err_emit( stk,
                                "load_all_images: invalid magic 0x%08x in '%s'",
                                magic, path );
                goto exit;
        }
        if ( rows != HERMES_IMG_ROWS || cols != HERMES_IMG_COLS ) {
                forge_err_emit(
                    stk, "load_all_images: unexpected dims %ux%u in '%s'", rows,
                    cols, path );
                goto exit;
        }
        ...
        if ( fread( raw, 1, total, f ) != total ) {
                forge_err_emit( stk, "load_all_images: short read in '%s'",
                                path );
                goto exit;
        }
...
static uint8_t *
load_all_labels( const char *path, size_t *n_out, struct err_stack *stk )
{
        int      ok     = 0;
        FILE    *f      = NULL;
        uint8_t *labels = NULL;
        ...
        if ( read_u32_be( f, &magic, stk ) || read_u32_be( f, &n, stk ) ) {
                forge_err_emit(
                    stk, "load_all_labels: failed reading header of '%s'",
                    path );
                goto exit;
        }
        if ( magic != 0x00000801u ) {
                forge_err_emit( stk,
                                "load_all_labels: invalid magic 0x%08x in '%s'",
                                magic, path );
                goto exit;
        }
        ...
        if ( fread( labels, 1, (size_t)n, f ) != (size_t)n ) {
                forge_err_emit( stk, "load_all_labels: short read in '%s'",
                                path );
                goto exit;
        }
        *n_out = (size_t)n;
...
static float *
load_image( const char *path, size_t index, struct err_stack *stk )
{
        int      ok    = 0;
        FILE    *f     = NULL;
        float   *img   = NULL;
        ...
        if ( read_u32_be( f, &magic, stk ) || read_u32_be( f, &n, stk ) ||
             read_u32_be( f, &rows, stk ) || read_u32_be( f, &cols, stk ) ) {
                forge_err_emit(
                    stk, "load_image: failed reading header of '%s'", path );
                goto exit;
        }
        if ( magic != 0x00000803u ) {
                forge_err_emit( stk, "load_image: invalid magic in '%s'",
                                path );
                goto exit;
        }
        if ( index >= (size_t)n ) {
                forge_err_emit(
                    stk, "load_image: index %zu out of range (max %u) in '%s'",
                    index, n - 1, path );
                goto exit;
        }
        (void)rows;
        ...
        for ( int i = 0; i < HERMES_PIXELS; i++ ) img[i] = raw[i] / 255.0f;
        ok = 1;
...
static int
load_label( const char *path, size_t index, struct err_stack *stk )
{
        int      rc    = -1;
        FILE    *f     = NULL;
        uint32_t magic = 0, n = 0;
        ...
        if ( read_u32_be( f, &magic, stk ) || read_u32_be( f, &n, stk ) ) {
                forge_err_emit(
                    stk, "load_label: failed reading header of '%s'", path );
                goto exit;
        }
        if ( magic != 0x00000801u ) {
                forge_err_emit( stk, "load_label: invalid magic in '%s'",
                                path );
                goto exit;
        }
        if ( index >= (size_t)n ) {
                forge_err_emit( stk,
                                "load_label: index %zu out of range in '%s'",
                                index, path );
                goto exit;
        }
...
static void
predict_one( size_t i, void *slot_out, const void *ctx )
{
...
static int
run_eval( const struct hermes_classifier *clf,
          ...
          const uint8_t *test_labels, size_t n )
{
        ...
        if ( forge_par_map( n, predict_one, preds, sizeof( uint8_t ), &ctx, 0,
                            &stk ) ) {
...

        {
                ...
                printf( "\nAccuracy: %zu/%zu (%.2f%%)\n", correct, n,
                        (double)correct / (double)n * 100.0 );
                printf( "\nPer-class breakdown:\n" );
                for ( int d = 0; d < 10; d++ ) {
                        printf( "  digit %d: %u/%u (%.2f%%)\n", d,
                                class_correct[d], class_total[d],
                                (double)class_correct[d] /
                                    (double)class_total[d] * 100.0 );
                }
        }
...
static int
run_knn( int argc, char **argv, struct err_stack *stk )
{
...
static int
run_nn( int argc, char **argv, struct err_stack *stk )
{
        ...
        printf(
            "Training MLP (hidden=%zu, lr=%.4f, epochs=%zu, batch=%zu)"
            " on %zu samples...\n",
